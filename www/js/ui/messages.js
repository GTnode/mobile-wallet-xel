...
.controller('messagesCtrl', function($scope, $state, $http, $ionicLoading, $rootScope) {
	...
	$http.get(shield.ADDRESS + '/nxt?requestType=getBlockchainTransactions&type=' + shield.TYPE_MESSAGING + '&subtype=' + shield.SUBTYPE_MESSAGING_ARBITRARY_MESSAGE + '&account=' + shield.globalAddress + '&timestamp=0')
    .success(function(response) {
		shield.database.removeCollection('senderdb');
		sendersDB = shield.database.addCollection('senderdb');
		shield.database.removeCollection('accountmessagesdb');
		accountmessagesDB = shield.database.addCollection('accountmessagesdb');
		$ionicLoading.hide();
		for (var i=0; i < response.transactions.length; i++) {
			var trans = response.transactions[i];
			accountmessagesDB.insert(trans);
			if(sendersDB.findOne({'senderRS': trans.senderRS}) == null && trans.senderRS != shield.globalAddress)
			{
...
$scope.sendMessage = function()
{
	...
	if($scope.plainText)
	{
		...
		messageFee = shield.FEE_NQT * messageFee;
		$ionicLoading.show({duration: 30000, noBackdrop: true, template: '<ion-spinner icon="spiral"></ion-spinner>'});
		shield.message_BuildHex(messageBytes, converters.stringToByteArray(""), shield.MESSAGE, $scope.recipient_address.text, messageFee, $rootScope.sendMessageCallBack);
	}
	else
	{
		if(shield.ADDRESS != "" && shield.ADDRESS != undefined ){
		...
		$http.get(shield.ADDRESS + '/nxt?requestType=getAccountPublicKey&account=' + $scope.recipient_address.text)
		.success(function(response) {
			$ionicLoading.hide();
			if(response.publicKey != 'undefined'){
				...
				messageFee = shield.FEE_NQT * messageFee;
				$ionicLoading.show({duration: 30000, noBackdrop: true, template: '<ion-spinner icon="spiral"></ion-spinner>'});
				shield.message_BuildHex(data.str, data.nonce, shield.ENCRYPTED_MESSAGE, messageFee, $scope.recipient_address.text, $rootScope.sendMessageCallBack);
			}
			else
			{
				var alertPopup = $ionicPopup.alert({
...
$rootScope.encryptNote = function(message, publicKey) {
	var privateKey = converters.hexStringToByteArray($scope.getPrivateKey(shield.globalPassPhrase));
...
if(shield.ADDRESS != "" && shield.ADDRESS != undefined ){
	...
	$http.get(shield.ADDRESS + '/nxt?requestType=getAccountPublicKey&account=' + showsenderRS)
    .success(function(response) {
		$ionicLoading.hide();
		...
		if(trans.attachment != undefined)
		{
			if(trans.attachment.encryptedMessage != undefined)
			{
				try
				{
					...
					var publicKey = "";
					if(trans.senderRS.indexOf(shield.globalAddress) != -1)
